	...
	def encode(self, X, **params):
		...
		if self.accelerated:
			...
			with Pool(min(self.task_number, self.max_pool_size)) as p:
				bitcode = p.starmap(_EBCOT_encode, inputs)
		else:
			bitcode = [_EBCOT_encode(self, x) for x in X]
...
def _band_encode(tile, bandMark, h=64, w=64, num=8):
	...
	for i in range(0, h_cA, h):
		for j in range(0, w_cA, w):
			...
			CX, D = _embeddedBlockEncoder(codeBlock, bandMark, h, w, num)
...
def _EBCOT_decode(path):
	...
	while bitcode.__len__() != 0:
		...
		deTiles.append(_tile_decode(bitcode[0:_index+1]))
...
def _band_decode(codestream, h=64, w=64, num=8):
	h_cA = codestream[0]
	w_cA = codestream[1]
...
def _decode_block(D, CX, h=64, w=64, num=8):
	...
	pointer = 0
	for i in range(num):
		V[i,:,:], signs, deS1, deS3, pointer = _SignificancePassDecoding(V[i,:,:], D, CX, deS1, deS3, pointer, signs, w, h)
